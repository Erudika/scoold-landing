...
{`
    ...
    @keyframes rowHighlight {
        0%, 80% { background-color: rgba(255, 200, 50, 0.25); }
        100%     { background-color: transparent; }
    }
    tr.row-highlight {
        animation: rowHighlight 7s ease forwards;
    }
    a.prop-anchor {
        color: inherit;
        text-decoration: none;
    }
    a.prop-anchor:hover::after {
        content: ' #';
        opacity: 0.4;
    }
`}
</style>
<script src="/scripts/row-highlight.js" is:inline></script>
...
|`scoold.app_secret_key`<br/>A random secret string, min. 32 chars long. *Must be different from the secret key of the Para app*. Used for generating API tokens (JWT) and passwordless authentication tokens. | ` ` | `String`|
...
|`scoold.api_user_access_enabled`<br/>Enable/disable the ability for users to get personal access tokens for accessing the Scoold API or MCP server. Disabled by default. | `false` | `Boolean`|
...
|`scoold.personal_token_expires_after`<br/>The validity period of personal API keys (Bearer tokens) for use with API/MCP. Setting this to 0 will show a select combobox in the UI. Default is 168h. | `604800` | `Integer`|
...
|`scoold.cors_enabled` <kbd>requires restart</kbd><br/>Enable/disable the CORS filter. It adds CORS headers to API responses. | `true` | `Boolean`|
|`scoold.cors_allowed_origins` <kbd>requires restart</kbd><br/>Comma-separated list of allowed origins for CORS. Leave empty to allow the same origin only. | ` ` | `String`|
...
|`scoold.security.ldap.spaces_attribute_name`<br/>LDAP attribute mapping for users' `spaces`. The spaces can be comma-separated. | `ou` | `String`|
|`scoold.security.ldap.token_delegation_enabled`<br/>Enable/disable LDAP attributes data delegation to Scoold. | `false` | `Boolean`|
...
|`scoold.security.saml.spaces_attribute_name` <kbd>Pro</kbd><br/>SAML attribute mapping for users' `spaces`. The spaces can be comma-separated. | `spaces` | `String`|
|`scoold.security.saml.token_delegation_enabled` <kbd>Pro</kbd><br/>Enable/disable SAML attributes data delegation to Scoold. | `false` | `Boolean`|
|`scoold.security.saml.groups_attribute_name` <kbd>Pro</kbd><br/>SAML attribute mapping for users' `groups`. Use this for mapping `admin`, `mod` and `user` roles to Scoold users. | ` ` | `String`|
|`scoold.security.saml.mods_equivalent_attribute_value` <kbd>Pro</kbd><br/>SAML attribute used for mapping SAML users having it, to moderators on Scoold. | `mod` | `String`|
|`scoold.security.saml.admins_equivalent_attribute_value` <kbd>Pro</kbd><br/>SAML attribute used for mapping SAML users having it, to administrators on Scoold. | `admin` | `String`|
|`scoold.security.saml.users_equivalent_attribute_value` <kbd>Pro</kbd><br/>SAML attribute used for **denying access** to SAML users **not** having it, *unless*they already have the admin or moderator roles assigned. | ` ` | `String`|
...
|`scoold.rate_limited_posting_enabled`<br/>Enable/disable rate limiting for new posts - 2 per min., 30 per hour, 50 per day (for questions)  and 2 per min., 50 per hour, 100 per day (for answers), per authenticated user. | `true` | `Boolean`|
...
|`scoold.default_starting_space`<br/>The starting space to be selected for all users upon sign in. | ` ` | `String`|
...
|`scoold.scholar_ifhas`<br/>Reputation points needed for earning the `scholar` badge. | `500` | `Integer`|
...
|`scoold.presigned_uploads_ttl_sec` <kbd>Pro</kbd><br/>Validity period for presigned links to uploaded files, in seconds. If set to 0, the feature is essentially disabled. If set to -1, presigned links never expire. Defaults is 600s. | `600` | `Boolean`|
...
|`scoold.blob_storage_key` <kbd>Pro</kbd><br/>Azure Blob Storage account key (Base64-encoded) for generating SAS tokens. | ` ` | `String`|
...
|`scoold.mathjax_enabled` <kbd>Pro</kbd><br/>Enable/disable support for MathJax/KaTeX/LaTeX for scientific expressions in Markdown. | `false` | `Boolean`|
|`scoold.mermaid_enabled`<br/>Enable/disable support for Mermaid diagrams in Markdown. | `false` | `Boolean`|
...
|`scoold.teams.bot_tenant_id` <kbd>Pro</kbd><br/>Teams bot tenant ID - this is separate from `ms_tenant_id` as that one is in conflict with some bot requests. | `common` | `String`|
...
|`scoold.import_via_s3_enabled`<br/>Enable/disable importing archives via S3 to overcome the request size limitations. | `false` | `Boolean`|
...
|`scoold.mcp_server_mode`<br/>Enable/disable the MCP server. Possible values: `off`, `r` - read-only, `rw` - full access. | `off` | `String`|
|`scoold.onboarding_enabled`<br/>Enable/disable the onboarding wizard when Scoold is initially started without any data. | `true` | `Boolean`|
