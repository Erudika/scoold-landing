...
You can also map LDAP DN nodes to Para user groups (roles). For example, with the following configuration:
```ini title="scoold-application.conf"
...
### Automatic space assignment from LDAP attributes

To allow Scoold to automatically pick up and assing spaces from LDAP attributes, enable attribute data delegation from Para to Scoold.
Also, specify which LDAP attribute contains the space name (e.g. `ou` - org. unit). The attribute value may contain comma-separated
list of spaces (departaments) which will be added to the user profile on login.

```ini title="scoold-application.conf"
scoold.security.ldap.token_delegation_enabled = true
scoold.security.ldap.spaces_attribute_name = "ou"
```
