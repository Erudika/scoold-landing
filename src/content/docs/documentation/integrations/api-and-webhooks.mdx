...
<Aside type="tip">
To revoke **all** existing API tokens, simply change the value of `scoold.app_secret_key` and restart Scoold.
Changing the Para secret key (`scoold.para_app_secret`) would log out all users.
</Aside>

### Personal API Tokens

API tokens generated from the **Administration** page are considered admin-level tokens and have full access permissions
for the Scoold API and MCP server. They are not linked to any user profile and actions performed through the API using
those tokens are recognized as if they were done by the "System" - a dummy user profile with ID `1`.

With the addition of the Scoold MCP server, we thought it would be nice to allow user to have their own personal tokens,
which would give AI agents the ability to interact with Scoold on behalf of the user.
This is where personal API tokens come in. This feature is disabled by default. To enable them, add this to your configuration:

```ini title=scoold-application.conf
scoold.api_user_access_enabled = true
```

That will unlock a new section on the **Settings** page, where each user can generate one API token at a time.
By default, the expiration period for personal tokens is 7 days, but that period is configurable.
Setting the period to `0` would allow users to select a period from a dropdown box on their settings page.

```ini title=scoold-application.conf
# expiration period in seconds, default is 7 days, 
# a value of 0 will show the dropdown box in the web UI
scoold.personal_token_expires_after = 604800
```
