...
When running Scoold behind a reverse proxy, it is recommended that you enable support for forward headers in Spring with
`server.forward-headers-strategy=NATIVE` in `application.properties` or as a system property.
</Aside>
